...
var SheetJSImportDirective = function () {
    return {
        scope: { opts: '=' },
        sessioStorage:{XLSX:'='},
        link: function ($scope, $elm, $attrs, $localStorage) {
            $elm.on('change', function (changeEvent) {
                ...
                reader.onload = function (e) {
                    ...
                    var data = [];
                    var j = 0;
                    for (var r = 0; r < dpk.length; ++r) {
                        data[r - 0] = {};
                        for (i = 0; i < dpk[r].length; ++i) {
                            if (dpk[r][i] == null) continue;
                            data[r - 0][$scope.opts.columnDefs[i].name] = dpk[j][i];
                            //data[r - 1][aoa[0][i]] = aoa[r][i]
                        }
                        j++;
                    }
                    //Get data in required Controller
